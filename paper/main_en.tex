...
\hypersetup{pdfauthor={Anonymous Author(s)},
            pdftitle={Hybrid Corrective RAG for Peruvian Medicinal Plants},
...
% ============================================================
\begin{abstract}
The vast existing literature on Peruvian medicinal plants suffers from a serious fragmentation problem. Being scattered across numerous repositories, extracting real pharmacological evidence is costly. To address this, we propose \sirca{} (System for Intelligent Retrieval and Corrective Answers), a bilingual architecture designed to unify this evidence through corrective techniques. Our solution fuses dense and lexical (BM25) search using \emph{Reciprocal Rank Fusion}, then applies a cross-encoder reranker that contributes in a directionally consistent way to answer fidelity. A CRAG-style router manages final quality. Our central empirical contribution is not a single retrieval score but a calibration finding: we show that the default within-batch normalization silently suppresses the CRAG corrective routes, and that an absolute sigmoid calibration is what makes fallbacks toward external search trigger legitimately. We evaluate the architecture in a real-world setting with 16{,}486 articles (covering 100 local species), on a bilingual benchmark of 50 questions with author-validated reference answers, attaining competitive metrics (Fidelity of $0{,}554$, MRR of $0{,}862$).
...
\caption{Full-pipeline headline metrics on the 50-query benchmark.}
\label{fig:headline}
\end{figure}
...
\midrule
Initial human-verified & 12 & $0{,}862$ & $0{,}801$ & $0{,}821$ & $0{,}474$ & $0{,}548$ \\
...
\caption{Retrieval generalization: quality holds up on 30 unseen species from the corpus.}
\label{fig:coverage}
\end{figure}
...
\caption{Robustness to swapping the generator: Gemma-4 wins only on Fidelity; DeepSeek wins only on BERTScore F1.}
\label{fig:cross_llm}
\end{figure}
...
\texttt{no\_crag}       & 0{,}496 & \textbf{0{,}573} & \textbf{0{,}889} & \textbf{0{,}857} & n/a & n/a \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Fidelity \texttt{full} vs.\ \texttt{no\_reranker}: the direction is consistent, but the final run does not reach conventional significance ($p{=}0{,}160$).}
...
\caption{Validation of CRAG's corrective branches on 27 out-of-distribution queries.}
