...
\textbf{Retrieval generalization.} Table~\ref{tab:extended_retrieval} shows that retrieval quality holds up — and in fact improves slightly — when the evaluation scope is widened to 30 unseen species from the corpus: on that set MRR rises from $0{,}866$ to $0{,}883$, NDCG@10 rises from $0{,}825$ to $0{,}879$, and Context Recall stays in the same range ($0{,}554 \rightarrow 0{,}550$). We do not interpret this as evidence that the 30 new species are inherently easier, but rather as confirmation that the retrieval engine is not overfit to the 12 human-verified species subset: it generalizes to unseen data without degradation.
...
\texttt{dense\_only}$^\dagger$ & 0{,}473 & 0{,}545 & 0{,}866 & 0{,}818 & n/a & n/a \\
\texttt{sparse\_only}   & \textbf{0{,}507} & \textbf{0{,}575} & \underline{0{,}825} & 0{,}813 & n/a & n/a \\
...
Removing pieces one by one (Table~\ref{tab:full_results}) revealed three useful truths. First, the reranking module is what protects Fidelity. When we turned it off, Fidelity dropped by $8{,}5\%$ relative (from $0{,}562$ to $0{,}514$), a statistically significant difference by paired Wilcoxon over $n{=}80$ queries ($p{=}0{,}028$; Sec.~\ref{sec:wilcoxon}). Second, keyword search alone (\texttt{sparse\_only}) shows a counterintuitive trade-off: it achieves the best C.Prec. ($0{,}507$) and C.Recall ($0{,}575$) of the five configurations, but the worst MRR ($0{,}825$) — BM25 brings more relevant documents into the 30-item basket, but without the dense component's semantic signal it ranks them worse before reranking. None of these differences reach per-query significance (Sec.~\ref{sec:wilcoxon}). Third, \texttt{dense\_only} does produce exactly the same Top-10 as \texttt{full} across all 50 queries ($n_{\neq}{=}0$): once the cross-encoder reranks, the initial bias toward pure dense retrieval becomes irrelevant — confirmed by a held-out $\alpha$ sweep (Section~\ref{sec:wilcoxon}) showing this invariance is not an artifact of the reported benchmark. Looking at the clock, retrieval takes us barely $\sim$1\,810\,ms to process on CPU, and drafting takes between 10 and 30 seconds using the DeepSeek server.
...
The statistical picture confirms that, at the retrieval level, no configuration differs significantly from \texttt{full}. \texttt{sparse\_only} fails to reach significance despite having the best aggregate C.Prec./C.Recall ($p{=}0{,}284$/$0{,}223$, $n_{\neq}{=}42$ on C.Recall): BM25 without the dense component wins on some queries and loses on others, and the net effect is indistinguishable from chance. \texttt{no\_reranker} likewise fails to reach significance on any retrieval metric ($p{\geq}0{,}27$ on all four). \texttt{no\_crag} differs from \texttt{full} in only $4$ of $50$ queries ($p{=}0{,}068$, borderline but not significant): disabling CRAG means those queries no longer trigger \textsc{refine}/\textsc{web\_search} and the final context changes; for the rest CRAG accepts the same Top-10. \texttt{dense\_only} does produce exactly the same Top-10 as \texttt{full} ($n_{\neq}{=}0$, marked with ``---''): the cross-encoder reranker makes final retrieval indifferent to which $\alpha$ the fusion started from. We confirmed this invariance is not an artifact of the reported benchmark with an $\alpha$ sweep over 30 held-out species never used to report numbers (\texttt{run\_alpha\_sweep\_heldout.py}): neither a fixed $\alpha$ other than $0{,}6$ nor a properly-tuned per-category $\alpha$ on that held-out set beat the current $\alpha{=}0{,}6$ when evaluated, once, on the 50 reported queries; that is why we removed the classifier's per-category weighting (Section~\ref{sec:architecture}) in favor of a fixed $\alpha$.

The overall reading is that, at the retrieval level, the five configurations are statistically equivalent to one another, and the architecturally central impact of the ablation lives in Fidelity, where removing the reranker costs an $8{,}5\%$ relative drop (Table~\ref{tab:full_results}). To verify that this drop is not noise, we applied a paired Wilcoxon signed-rank test on per-query Fidelity between \texttt{full} and \texttt{no\_reranker} (with DeepSeek generation). On the 50-query reported benchmark, the effect did not reach conventional significance ($p{=}0{,}110$ two-sided). Since Fidelity does not require a reference answer, we extended the set to $n{=}80$ by adding the 30 held-out species, and with that added power the difference was significant ($p{=}0{,}028$ two-sided; $p{=}0{,}014$ one-sided; all 80/80 queries registering a non-zero difference). We document both runs explicitly: the direction of the effect (\texttt{full} $>$ \texttt{no\_reranker}) held stable across the three runs performed during this work ($0{,}534{>}0{,}467$; $0{,}566{>}0{,}465$; $0{,}562{>}0{,}514$), but formal significance oscillated between $p{=}0{,}016$, $p{=}0{,}00023$, and $p{=}0{,}110$ depending on the run, which we discuss as a statistical-power limitation in Limitation~(ix).

\textbf{Note on runs and the absolute value of Fidelity.} DeepSeek's Fidelity in the cross-LLM comparison (Table~\ref{tab:multi_llm}, $0{,}507$) differs from \texttt{full}'s Fidelity in Table~\ref{tab:full_results} ($0{,}562$) because they come from different runs of the commercial API: Table~\ref{tab:multi_llm} uses the paired run with Gemma-4, while Table~\ref{tab:full_results} uses a dedicated ablation run. The one result reproducible across runs — and the one that supports the article's conclusion — is the \emph{direction} of the differences, not their absolute value, and (as documented above) not always their formal significance either.
