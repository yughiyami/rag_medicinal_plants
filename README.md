...
A Corrective RAG (CRAG) system that answers questions about Peruvian medicinal plants using scientific literature. It retrieves evidence from a curated corpus, evaluates relevance, and generates grounded answers with citations.

**Key contribution:** A hybrid retrieval pipeline (dense + sparse + cross-encoder reranking) with corrective evaluation that improves answer faithfulness over vanilla RAG in the ethnobotanical domain.
...
|-------|-----------|---------|
| **Classify** | Rule-based classifier | factual / exploratory / comparative |
| **Retrieve** | Dense (e5-multilingual 768d) + BM25 sparse | Fused via Reciprocal Rank Fusion, reranked with cross-encoder |
| **Evaluate** | CRAG threshold | accept (>=0.35), refine (0.15-0.35), web_search (<0.15) |
| **Generate** | Chain-of-Thought grounding | Extract -> Verify -> Compose. DeepSeek, Ollama, or template backend |
...
HF_HOME=D:\MLCache\huggingface           # HuggingFace model cache
```
...
sirca_rag/
  ...
  data/
    ...
    processed/               # chunks_expanded.json (82K chunks)
...
| e5-multilingual | Bilingual EN/ES support, practical CPU encoding speed |
